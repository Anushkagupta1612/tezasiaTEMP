...
class MintNft:
    ...
    def awarditem(self, batch):
        ...
        # sp.verify(self.is_administrator(sp.sender), "FA2_NOTADMIN")
        with sp.for_("action", batch) as action:
            ...
            self.data.ledger[token_id] = action.to
...
if "templates" not in __name__:
    ...
    class FungibleTest(
        ...
        def __init__(self, **kwargs):
            Fa2Fungible.__init__(self, **kwargs)
            Admin.__init__(self, admin.address)
    ...
    class SingleAssetTest(
        ...
        """SingleAsset contract with all optional features."""

        def __init__(self, **kwargs):
            Fa2SingleAsset.__init__(self, **kwargs)
            Admin.__init__(self, admin.address)
    ...
    # Standard features
    for _Fa2 in [Fa2Nft, Fa2Fungible, Fa2SingleAsset]:
    ...
    for _Fa2 in [NftTest, FungibleTest, SingleAssetTest]:
        token_metadata = tok0_md if _Fa2.ledger_type == "SingleAsset" else []
        TESTS.NS.test_admin(_Fa2(metadata=METADATA))
        TESTS.NS.test_mint(_Fa2(metadata=METADATA, token_metadata=token_metadata))
